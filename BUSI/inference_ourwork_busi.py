...
def main(args):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="处理进度"):
        if "normal" in str(row.get("img", "")).lower():
            continue
        ...
        try:
            ...
            tqdm.write(f"⚠️ Failed id={row.get('id','')}: {str(e)}")
            continue
...
if __name__ == "__main__":
    ...
    repo_root = REPO_ROOT
    busi_root = repo_root / "BUSI"

    ap.add_argument("--ckpt", default=str(repo_root / "work_dir" / "MedSAM" / "medsam_vit_b.pth"))
    ap.add_argument("--model_type", default="vit_b")

    ap.add_argument("--meta", default=str(busi_root / "after_processor" / "meta.csv"))
    ap.add_argument("--img_dir", default=str(busi_root / "after_processor" / "images_1024"))
    ap.add_argument("--mask_dir", default=str(busi_root / "after_processor" / "masks_1024"))
    ap.add_argument("--out", default=str(busi_root / "inference_ourwork_results"))
    ...
    ap.set_defaults(save_pred=False)
